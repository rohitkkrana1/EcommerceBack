...
def update_cart():
	...
	qty = data['qty']
	additional_notes = len(data['additional_notes']) > 1 and data['additional_notes'] or None
	with_items = len(data['with_items']) > 1 and data['with_items'] or False
...
def _get_cart_quotation(party=None):
	...
	quotation = frappe.get_all(
		...
		filters={
			...
			"contact_email": frappe.get_user().doc.name,
	...
	if quotation:
		...
		qdoc.contact_person = frappe.db.get_value("Contact", {"email_id": frappe.get_user().doc.name})
		qdoc.contact_email = frappe.get_user().doc.name
